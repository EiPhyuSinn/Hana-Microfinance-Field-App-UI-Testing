...
class InterviewProcessHelper {
	...
	async clientInfoPage() {
        ...
        // if (await phoneInput.getText() === "") {
            await Util.choosePhoneNumber("788000177");
        // }
	...
	async householdVerificationPage() {
		...
		const desiredLabelElement = await $('//*[@text="အိမ်အကျယ်အဝန်း - အလျားပေ *"]');
         
		// while (!(await desiredLabelElement.isExisting())) {
		// 	await Util.scrollToText("အိမ်အကျယ်အဝန်း - အနံပေ *")
		// }
		await Util.scrollToText("အိမ်အကျယ်အဝန်း - အနံပေ *")
		...
		for (const inputBox of inputBoxList) {
			if ((await inputBox.getText()) == "") {
				//await inputBox.setValue(Math.floor(Math.random() * 100 + 15))
				await inputBox.setValue(50)
			}
		}
	...
	async businessProfilePage(interviewType) {
		...
		if (interviewType == "individual") {
			const desiredLabel = "လျှောက်ထားသူမအားလပ်ချိန်တွင် လုပ်ငန်းလည်ပတ်နိုင်မှု ရှိ၊မရှိ *"
	...
	async evaluationPageIndividual(input) {
		...
		if (await approvedLoanAmount.getText() === '') {
			...
			await approvedLoanAmount.setValue(input ? input.fo_approved_amount : '800000');
		}
