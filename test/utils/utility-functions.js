...
class Utility {
	...
	async customScroll(xStart, yStart, xEnd, yEnd, duration) {
		await browser.action('pointer', {
		  ...
		  actions: [
			{ type: 'pointerDown', x: xStart, y: yStart },  // Press at start coordinates
			{ type: 'pause', duration: duration },        // Wait for specified duration
			{ type: 'pointerMove', x: xEnd, y: yEnd },    // Move to end coordinates
			{ type: 'pointerUp' },                          // Release the touch gesture
		  ],
		});
	  }
	...
	async choosePhoneNumber() {
		...
		if (randomNumber % 2 === 0) {
			...
			await phoneNumberInput.setValue("788000177")
